...
function RectanguloDeformado(){
  return <svg className="absolute lg:w-[350px] lg:h-[270px] w-[260px] h-[220px] -top-10 -right-10" viewBox="0 0 262 223" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function Message_login(){
  return <p className='text-gray-500'>Already have an account? <Link className='text-[rgb(58,3,94)]' to="/">Log in</Link></p>
}


function Campo_nombre({error, cambiarError}){
  function cambiartexto(e){
    if (e.target.value.trim() !== '') {
      cambiarError("");
    } else {
      cambiarError('El nombre es obligatorio');
    }
  }
  ...
  return <div className='flex flex-col gap-y-1'>
      <div className={`lg:h-[60px] lg:w-[300px] h-[50px] w-[280px] flex justify-start items-center gap-x-4  ${error !== '' ? "border-2 border-red-500" : " border-0"} rounded-[20px] ps-5 shadow-[0px_0px_10px_3px] shadow-shadow-campos/25`}>
        <label className="h-[30px] w-[30px]" htmlFor="nombre">
        ...
        <input onChange={cambiartexto} className="placeholder-black placeholder:text-xl placeholder:font-medium placeholder:font-sans" type="text" id="nombre" name="nombre" required placeholder="Name"></input>
    </div>
    {error !== '' ? texto_error : ""}
  </div>
}
...
function Campo_email({error, cambiarError}){
  ...
  return <div className='flex flex-col gap-y-1'>
      <div className={`lg:h-[60px] lg:w-[300px] h-[50px] w-[280px] flex justify-start items-center gap-x-4 ${error !== "" ? "border-2 border-red-500" : "border-0"} rounded-[20px] ps-5 shadow-[0px_0px_10px_3px] shadow-shadow-campos/25`}>
        ...
        <input onChange={cambiarTexto} className="placeholder-black placeholder:text-xl placeholder:font-medium placeholder:font-sans" type="email" id="email" name="email" required placeholder="Email"></input>
    </div>
    {error !== '' ? texto_error : ""}
  </div>
}
...
function Campo_password({error, cambiarError}){
  ...
  return <div className='flex flex-col gap-y-1'>
      <div className={`lg:h-[60px] lg:w-[300px] h-[50px] w-[280px] flex justify-start items-center gap-x-4 ${error ? "border-2 border-red-500" : "border-0"} rounded-[20px] ps-5 shadow-[0px_0px_10px_3px] shadow-shadow-campos/25`}>
        ...
        <input onChange={cambiartexto} className="placeholder-black placeholder:text-xl placeholder:font-medium placeholder:font-sans" type="password" id="password" name="password" required placeholder="Password"></input>
    </div>
    {error !== '' ? texto_error : ""}
  </div>
}

function Boton_Create_Account({error}){
  ...
  return <div>
      <div className="lg:h-[50px] lg:w-[300px] h-[40px] w-[280px] flex justify-center items-center rounded-[20px]  bg-gradient-to-r from-botones-registro-morado to-botones-registro-azul">
        <button type="submit" className="h-full w-full font-bold text-white text-xl     font-sans rounded-[20px]">Create Account</button>
    </div>
    {error !== "" ? texto_error : ""}
  </div>
}
...
function Form(){
  ...
  const [errorExistente, setError] = useState("");

  //Para ir al login cuando se cree el usuario
  const navigate = useNavigate();

  async  function obtener_datos(evento){
    evento.preventDefault();

    // Evita enviar si hay errores de validación activos
    ...
    try {
      ...
      //Cuando se crea el usuario la pagina se cambia al registro
      if (mensaje.status === 201) {
        navigate("/");
        return;
      }

      //Se obtiene el json del objeto response
      const respuesta = await mensaje.json();

      //Si un campo no fue llenado se imprime el error en pantalla
      if (!mensaje.ok) {
        if (respuesta?.errores) {
          for (let i = 0; i < respuesta.errores.length; i++) {
            if (respuesta.errores[i].path === "nombre") setNombreError(respuesta.errores[i].msg);
            if (respuesta.errores[i].path === "email") setEmailError(respuesta.errores[i].msg);
            if (respuesta.errores[i].path === "password") setPasswordError(respuesta.errores[i].msg);
          }
        }
        throw new Error(respuesta.mensaje);
      }
    } catch (error) { 
      if (error.message === "Error 1062") {
        setError("El nombre de usuario o el correo ya esta en uso");
      } else {
        setError("");
      }
    }
  ...
  return <form noValidate onSubmit={obtener_datos} className="flex flex-col justify-center items-center gap-y-9">
      <TextoRegistro texto={"Sign Up"}></TextoRegistro>
      <div className={`flex flex-col justify-center items-center gap-y-5`}>
        <Campo_nombre error={nombreError} cambiarError={setNombreError}></Campo_nombre>
      ...
      <Boton_Create_Account error={errorExistente}></Boton_Create_Account>
