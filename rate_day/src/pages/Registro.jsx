...
function Form(){
      ...
      if (!mensaje.ok) {
        if (Object.hasOwn(respuesta, "errores")) {
          for (let i = 0; i < respuesta.errores.length; i++) {
            console.log(respuesta.errores[i].path);
            if (respuesta.errores[i].path === "nombre") {
              setNombreError(respuesta.errores[i].msg);
            } else if (respuesta.errores[i].path === "email") {
  ...
  return <form noValidate onSubmit={obtener_datos} className="flex flex-col justify-center items-center gap-y-9">
      <Texto_registro texto={"Sign Up"}></Texto_registro>
